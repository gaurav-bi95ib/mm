<?php
// MeroMaidan - Player Notifications Inbox
...
$stmt = $db->prepare("SELECT * FROM notifications WHERE user_id = ? AND role = 'player' ORDER BY created_at DESC");
...
<head>
  ...
  <title>Notifications - MeroMaidan Player</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', system-ui, sans-serif; }
    ...
    .notif-card { background: white; border-radius: 10px; padding: 16px 20px; border-left: 4px solid #f97316; box-shadow: 0 2px 5px rgba(0,0,0,0.03); display: flex; gap: 16px; align-items: flex-start; }
    .notif-card.unread { background: #fff7ed; border-left-color: #f97316; }
    .notif-card.read { border-left-color: #cbd5e1; opacity: 0.8; }
    .notif-icon { font-size: 20px; background: #ffedd5; padding: 10px; border-radius: 50%; }
...
<div class="sidebar">
  ...
  <a href="favorites.php" class="nav-item">❤️ Favorites</a>
